...
open class FRadioPlayer: NSObject {
    ...
    var totalAudioDuration:((_ value:String)->Void)?
    var currentTimeAudio:((_ value:Double)->Void)?
    
    
    var totalDurationDouble:Double = 0.00
    ...
    func addPeriodicTimeObserver() {
        ...
        timeObserverToken = player.addPeriodicTimeObserver(forInterval: time,
                                                           queue: .main) {
                                                            ...
                                                            if self!.totalDurationDouble > 0.00
                                                            {
                                                                
                                                                let thisTime = time.seconds/self!.totalDurationDouble
                                                                self!.currentTimeAudio!(thisTime)
                                                            }
        ...
        if let item = object as? AVPlayerItem, let keyPath = keyPath, item == self.playerItem {
            ...
            case "status":
                ...
                if player?.status == AVPlayer.Status.readyToPlay {
                    self.totalDurationDouble = playerItem?.duration.seconds as! Double
                    self.totalAudioDuration!(playerItem?.duration.durationText ?? "00:00")
