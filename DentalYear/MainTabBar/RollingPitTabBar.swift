...
@IBDesignable class RollingPitTabBar: UITabBar {
    ...
    @IBInspectable public var barHeight : CGFloat = 45 //bar heigh
    ...
    let marginLeft : CGFloat = 0 //left margin of bar
    
    let marginRight : CGFloat = 0 //right margin of bar
    ...
    private var barRect : CGRect{
        get{
            let h = self.barHeight + circleRadius
    ...
    private lazy var backgroundMask : CAShapeLayer = {
        let result = CAShapeLayer()
        result.fillRule = CAShapeLayerFillRule.nonZero
        return result
    }()
    ...
    private func positionItem(barRect : CGRect, totalCount : Int, idx : Int, item : UITabBarItem, view : UIView){
        ...
        if self.selectedItem == item {
            ...
            y = barRect.origin.y - (self.circleRadius / 2) + 3
        }
    ...
    private func layoutElements(selectedChanged : Bool){
        ...
        for (idx, item) in items.enumerated() {
            if selectedChanged {
                if self.selectedItem == item.item {
                    item.item.titlePositionAdjustment = UIOffset(horizontal: 0, vertical: 5)
                }else
                {
                    item.item.titlePositionAdjustment = UIOffset(horizontal: 0, vertical: 5)
                }
