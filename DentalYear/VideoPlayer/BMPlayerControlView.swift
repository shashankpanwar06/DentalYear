...
open class BMPlayerControlView: UIView {
    ...
    var fullScreenButtonPressed:((_ fullScreen:Bool)->Void)?
    ...
    open func playTimeDidChange(currentTime: TimeInterval, totalTime: TimeInterval) {
        totalTimeLabel.text = BMPlayer.formatSecondsToString(currentTime)
    ...
    open func showSeekToView(to toSecound: TimeInterval, total totalDuration:TimeInterval, isAdd: Bool) {
        ...
        totalTimeLabel.text = targetTime
    }
    ...
    open func controlViewAnimation(isShow: Bool) {
        ...
        UIView.animate(withDuration: 0.3, animations: {[weak self] in
            ...
            if isShow {
                ...
                if wSelf.player?.isPlaying ?? false {
                    wSelf.replayButton.isHidden = true
                }else
                {
                    wSelf.replayButton.isHidden = false
    ...
    open func showCover(url: URL?) {
        ...
        if let url = url {
            ...
            self.maskImageView.sd_setImage(with: url) { (image, error, type, url) in
                self.hideLoader()
            }
...
//            }
    ...
    func changestateFullScreen()  {
        ...
        if let callback = fullScreenButtonPressed
        {
            callback(fullscreenButton.isSelected)
        }
    }
    func changestateMute()  {
    ...
    func setupUIComponents() {
        ...
        bottomWrapperView.backgroundColor = UIColor.black.withAlphaComponent(0.7)
        ...
        bottomWrapperView.addSubview(muteButton)

        
        playButton.tag = BMPlayerControlView.ButtonType.play.rawValue
        playButton.setImage(UIImage.init(named: "icPlayVideo"),  for: .normal)
        playButton.setImage(UIImage.init(named: "icPauseVideo"), for: .selected)
        playButton.addTarget(self, action: #selector(onButtonPressed(_:)), for: .touchUpInside)
        
        currentTimeLabel.textColor  = UIColor.white
        currentTimeLabel.font       = UIFont.init(name: "MontserratMedium", size: 12) ?? UIFont.systemFont(ofSize: 12)
        currentTimeLabel.text       = "00:00"
        currentTimeLabel.textAlignment = NSTextAlignment.center
        
        totalTimeLabel.textColor    = UIColor.white
        totalTimeLabel.font         = UIFont.init(name: "MontserratMedium", size: 12) ?? UIFont.systemFont(ofSize: 12)
        ...
        timeSlider.setThumbImage(UIImage.init(named: "thumbVideoSlider"), for: .normal)
        
        timeSlider.maximumTrackTintColor = UIColor.white
        timeSlider.minimumTrackTintColor = UIColor.hexStringToUIColor(hex: "#A5E5FC")
        ...
        fullscreenButton.setImage(UIImage.init(named: "exitFullScreen"), for: .selected)
        ...
        muteButton.tag = BMPlayerControlView.ButtonType.mute.rawValue
         muteButton.setImage(UIImage.init(named: "isSound"),    for: .normal)
         muteButton.setImage(UIImage.init(named: "icVolumeMute"), for: .selected)
         muteButton.addTarget(self, action: #selector(onButtonPressed(_:)), for: .touchUpInside)
        ...
        replayButton.isHidden = false
        replayButton.setImage(UIImage.init(named: "iclargePlay"), for: .normal)
    ...
    func addSnapKitConstraint() {
        ...
        timeSlider.snp.makeConstraints { [unowned self](make) in
            make.centerY.equalTo(self.playButton)
            make.left.equalTo(self.playButton.snp.right).offset(10).priority(750)
            make.height.equalTo(30)
        }
        ...
        muteButton.snp.makeConstraints { [unowned self](make) in
            make.width.equalTo(50)
            make.height.equalTo(50)
            make.centerY.equalTo(self.playButton)
            make.left.equalTo(self.totalTimeLabel.snp.right)
            make.right.equalTo(self.muteButton.snp.left)
        }
        ...
        replayButton.snp.makeConstraints { [unowned self](make) in
            make.centerX.equalTo(self.mainMaskView).offset(0)
            make.centerY.equalTo(self.mainMaskView).offset(-20)
