...
open class BMPlayer: UIView {
    ...
    open func setVideo(resource: BMPlayerResource, definitionIndex: Int = 0) {
        ...
        if BMPlayerConf.shouldAutoPlay {
            ...
            //            playerLayer?.makeReadyToPlay(asset: asset.avURLAsset)
    ...
    open func play() {
        ...
        self.controlView.hidePlayToTheEndView()
...
//        controlView.updateUI(!self.isFullScreen)
    ...
    @objc fileprivate func muteButtonPressed() {
        
        guard let muted = self.avPlayer?.isMuted else {
